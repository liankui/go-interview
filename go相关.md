...
map是一个kv对集合。底层使用的是哈希查找表，并且使用链表解决哈希冲突的问题。
...
golang释放map内存的方法：首先删除map中的所有key，map占用内存仍处于【使用状态】；然后将map设置为nil，map占用的内存处于【空闲状态】；最后处于空闲状态内存，一定时间内在下次申请的可重复被使用，不必再向操作系统申请即可。
...
- 扩容完毕后（预分配），不会马上就进行迁移。而是采取增量扩容的方式，当有访问到具体 bucket 时，才会逐渐的进行迁移（将 oldbucket 迁移到 bucket）
